...
def request_fit(method, url, max_retry = 0, cookie = None, stream = False, customized_headers = None):
	headers = customized_headers if customized_headers else {
...
def get_resources(uid, video, interval, limit, token):
	...
	if (limit[0] == limit[1]):
		print('single fetch: ', limit[0])
		try:
			url = 'https://m.weibo.cn/statuses/show?id={}'.format(limit[0])
			response = request_fit('GET', url, cookie = token)
		...
		try:
			...
			for card in cards:
				...
				if 'mblog' in card:
					...
					if 'newest_bid' not in info: #Save newest bid
						info['newest_bid'] = mblog['bid']
					if compare(limit[0], '>=', [mid, date]) and not passtop: # 跳过置顶
						#print('置顶：', limit[0], [mid, date])
						continue
					if compare(limit[1], '<=', [mid, date]): 
						#print('晚于:', limit[1], [mid, date])
						passtop = True
					if compare(limit[0], '>=', [mid, date]):
						print('早于:', limit[0], [mid, date], ', 超出中断')
						exceed = True
					if compare(limit[0], '>=', [mid, date]) or compare(limit[1], '<', [mid, date]): continue # 未到跳过
					if 'pics' in mblog:
						if mblog['pic_num'] > 9: # More than 9 images
							blog_url = card['scheme']
							print_fit(f'Find more than 9 pictures for {blog_url}!')
...
def download(url, path, overwrite):
	...
	download_heads = {
		...
		'Accept-Language': 'zh-CN,en-US;q=0.8,en;q=0.5,zh-TW;q=0.3',
	...
	try:
		with request_fit('GET', url, stream = True, customized_headers = download_heads) as response:
		...
		global totalmbytes
		totalmbytes += kbytes
		print_fit('[{}] {} -> {} [{} KB] finished in {} seconds, {} K/s, total {} KB downloaded.'.format(ends, response.url, path.name, kbytes, secs, kbps, round(totalmbytes/1000.0, 1)))
		return True
